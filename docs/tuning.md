...
| `PASSING_LANES[].catchWidth`                    | 0.17, 0.15, 0.14, 0.13 | Larger is more forgiving horizontally. Catch height remains `0.08..0.92` in `collision.ts`.                                    |
...
| `throw.catchProgress`                           |                   0.82 | Portion of the extended trajectory at which the ball reaches the destination X. Lower values carry farther beyond the marker.  |
