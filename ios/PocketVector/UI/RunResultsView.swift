...
struct RunResultsCoinLedger: Equatable {
    ...
    init(results: RunResultsPresentation) {
        total = max(0, results.earnedCoins)

        guard results.completedRun.configuration.economyVersion == EconomyConfiguration.currentVersion else {
            lines = [RunResultsCoinLine(id: "run-reward", label: "RUN REWARD", amount: total)]
            return
        }

        let breakdown = RunRewardCalculator.calculate(for: results.completedRun)
        let signingBonus = total - breakdown.totalCoins
        guard breakdown.isEligible, signingBonus >= 0 else {
            lines = [RunResultsCoinLine(id: "run-reward", label: "RUN REWARD", amount: total)]
            return
        }
        ...
        var earnedLines = [
            RunResultsCoinLine(
                id: "run-complete",
                label: "RUN COMPLETE",
                amount: breakdown.completionCoins
            ),
            RunResultsCoinLine(
                id: "score-bonus",
                label: "SCORE BONUS",
                amount: breakdown.performanceCoins
            ),
            RunResultsCoinLine(
                id: "accuracy-bonus",
                label: "ACCURACY BONUS",
                amount: breakdown.accuracyCoins
            ),
        ]
        if signingBonus > 0 {
